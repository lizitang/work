$(function(){
	...
	function creatTable(tableHead,bodyData,saveData){
		...
		for(var i = 0 ;i<serialNumber.length;i++){
			...
			var tdHtml='<td colspan = "4" style="padding:10px 30px;">'+
						'<div style = "width:70%;overflow:hidden;" class = "contentBox" id="myDiv">'+
						'</div>'+
		...
		$(document).on('mouseup',".highlight",function(e){
			lock=true;
		 	...
		 	$(document).on("mouseup",'.dialog',function(){
    			/*console.log(this);*/
            	TextHighlight();
            	length=$('.contentBox').find('span.light').length;
    ...
    function init()
    {
        $(document).on("contextmenu",'.dialog',function(){
			//屏蔽掉浏览器本身的右键菜单
...
　　　　　　event.preventDefault()
    ...
    function TextHighlight(obj) {
		...
		var info=[selectText,anchorOffset,focusOffset];
		console.log(info)
		lock=false;
		return info;
    }
